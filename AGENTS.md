...
- 本地门禁 `npm run verify`：vitest 51 files / 305 passed / 0 failed；typecheck / lint: 0 errors、0 warnings（干净检出口径；本机 `.artifacts/` 下的未跟踪脚本会多出 1 条 warning，CI 看不见它）
...
- `.artifacts/` / `.worktrees/` / `.omo/` / `.codegraph/` / `.workbuddy/` 都是本机工具产物，已在 `.gitignore` 里，保持未跟踪、勿入库（原 [docs/PLAN.md](docs/PLAN.md) 的「Git 状态」段并入这条）
