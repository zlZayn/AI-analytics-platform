...
2. 配置 AI 后生成洞察，确认 SQL 只引用当前 Schema；未知图表或错误字段安全回退到表格。
...
3. 切换亮色与深色并刷新，确认主题保持、文字和图表可读、无首屏明显闪烁。
   特别检查连接管理页、结果表、图表画布、提示条、弹层和 SVG 坐标文字，不得残留突兀白底。
4. 仅使用键盘完成连接选择、查询执行、图表切换、保存和对话框取消；焦点必须始终可见。
5. 开启系统“减少动态效果”，确认动画被抑制；浏览器缩放到 200%，确认文字不遮挡操作按钮。
6. 检查 Console 无报错，Network 中失败请求都能结束，界面不存在永久 loading。

## 通过标准

上述步骤全部通过，且真实业务数据的图表含义与 SQL 聚合含义一致，才可视为本轮人工验收完成。任何失败都应记录页面、连接、SQL、视口、主题、复现步骤和 request-id。
