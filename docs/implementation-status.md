...
更新时间：2026-09-02（重构 v4.5 全阶段完成）

当前产品版本：`1.24.0`（refactor-plan 分支，未合并 main）

## Git 状态

- 工作分支：`refactor-plan`（重构主线，稳定后合并 main）
- 重构方案：[docs/REFACTOR_PLAN_v4.md](REFACTOR_PLAN_v4.md) · 进度追踪：[REFACTOR_STATUS.md](../REFACTOR_STATUS.md)
- 前一阶段收尾：`main` 上的 `34a9651`（workbench refactor 合并）+ `b5aa748`（query lifecycle）+ `9779185`（收尾硬化）
