...
- 工作台初始 SQL 由规范化导航 payload 驱动，按 `connection + SQL` key 幂等填充编辑器并执行，兼容客户端导航参数晚到
- 执行状态和错误提示可见；结果区固定容器承载等待、执行中、成功、失败状态，成功结果不再嵌套重复外框
- `/explorer` 与 `/queries` 入口均纳入离线 E2E，断言 SQL 参数、编辑器填充和每次导航仅一次 `/api/query`
