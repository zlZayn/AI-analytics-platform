...
5. 统一历史时间线：在同一连接下执行若干 SQL、提问 AI、运行 R 后，打开「查询管理 → 历史」，确认三类条目按时间倒序混排且带 SQL/AI/R 徽标；搜索框同时匹配三者；SQL 与带来源 SQL 的 R 条目有「执行」（R 点执行应带回工作台并自动开面板回放该条代码），无可执行回退 SQL 的 AI 条目只给「复制问题」。
