...
5. 统一历史时间线：在同一连接下执行若干 SQL、提问 AI、运行 R 后，打开「查询管理 → 历史」，确认三类条目按时间倒序混排且带 SQL/AI/R 徽标；搜索框同时匹配三者；SQL 与带来源 SQL 的 R 条目有「执行」（R 点执行应带回工作台、自动开面板并**重新执行该条代码出图**，含图条目显示「含图 N 张」），无可执行回退 SQL 的 AI 条目只给「复制问题」。
6. 历史持久化：执行 SQL/AI/R 各一次后重启服务（换端口/换浏览器同样）再打开「查询管理 → 历史」，确认三类条目都在——AI/R 权威源是元库 `analysis_history` 表（见[决策记录](../.agents/notes/2026-09-11-history-persistence-and-r-replay-rerun.md)）。
...
7. 历史回放：在「查询管理 → 历史」点某条 R 条目的「执行」，确认面板自动打开并**自动重跑该条代码**（旧输出与旧图先被清空，出图与文本都是这次执行的结果）；纯绘图条目（无文本输出）也要出现在历史里。
8. 面板关闭后查询与图表主链路不受影响；刷新页面后 R 运行时需重新下载（预期行为）。
