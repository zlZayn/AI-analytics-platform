...
下方是查询结果区域：10 种基础视图、可解释推荐、合法默认映射、显式堆叠/相关方法选项和窗口化结果表。

### 3. 数据探索 `/explorer`

宽屏左侧是表列表、右侧是字段/关联/预览详情；窄屏按列表到详情分层。预览只向服务端传 schema/table 标识符，服务端安全引用并限制 100 行。选中表后可点击“在工作台执行”跳转并预填 SQL。
