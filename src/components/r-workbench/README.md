...
- **历史**：每次执行落一条统一历史（`lib/history-store.ts`，**作用域键 = 连接 id**，携带 `sessionId` 溯源与 `sourceSql` 回放锚点；文本输出可回放、图片不持久化）；重开面板优先恢复上次代码，但**去掉旧的数据加载块并用当前数据集重建 `df`**（否则会用旧数据），文本输出只在面板生命周期内回放一次。
- **历史回放**：查询管理页「执行」带 `?r=<id>` 时，结果就绪后自动打开面板并载入该条代码（按 id 从统一历史取回，只消费一次）；无 `sourceSql` 的旧记录不提供回放入口。
