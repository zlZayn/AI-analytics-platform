...
- `components/`：`layout/` 布局与连接上下文；`charts/` 图表系统（views/ 视图、algorithms/transform 管线）；`dashboard/` 结果面板；`workspace/` 会话工作台；`SessionView.tsx` 结果区三层 Tabs（洞察 InsightCard 卡片流 / 探索 ResultPanel / 明细 table-view）；`ResultPanel` + `ChartConfigPanel.tsx` 图表配置面板（类型网格/列映射槽位/图例开关）；`InsightCard.tsx` AI 洞察卡片（洞察 Tab 卡片流，AI 多结果全量消费）；`AiVisibilityHint.tsx` AI 可见性提示（用户可见边界）；`AiMentionInput.tsx` + `AiMentionPanel.tsx` @ 提及选表（显式上下文注入）；`ResultToolbar.tsx` 结果「导出 ▼」下拉（CSV/JSON/R 模板/R 分析）；`r-workbench/` R 分析工作台（8 文件：容器/头部/工具栏/编辑器/输出/输出项/图像/状态栏）；`ui/` 基础组件（`select`/`card`/`table` 为成品备件，当前零引用可复用）
...
- `lib/`：核心库（AI 服务、查询编译器、校验器、渲染绑定、查询引擎、SQL 校验、Schema 扫描、数据轮廓、加密、池管理、`mention.ts` @ 提及纯逻辑、`workspace-navigation.ts` 工作台 URL 构造、`r-bridge.ts` R 数据桥（SemanticDataset↔R 代码/CSV 纯函数）、`webr-client.ts` WebR 客户端（非 React 单例类）、`monaco-setup.ts` 本地 monaco 配置）
